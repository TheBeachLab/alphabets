...
def _captured_bambu_a1_mini_plate(
    ...
    upper_preview = _place_on_plate(
        parts["enclosure_upper"], x=0, y=0, x_rotation=-90
    )
    lower_preview = _place_on_plate(
        parts["enclosure_lower"], x=0, y=0, x_rotation=-90
    )
    ...
    pawl_y = group_y + (
        lower_size.ylen - max(definitive_size.ylen, prototype_size.ylen)
    ) / 2
    placements = {
    ...
    for index, first in enumerate(placement_values):
        for second in placement_values[index + 1 :]:
            if first.intersect(second).Volume() > 1e-6:
                raise ValueError("captured enclosure parts overlap on the A1 mini plate")
    ...
    for name, shape in placements.items():
        ...
        layout[name] = {
            ...
            "bed_face": (
                "front enclosure rim" if name.startswith("enclosure") else "flat rear face"
            ),
            "rotation_deg": (
                {"x": -90, "z": 0}
...
def generate_captured_enclosure(
    ...
    for half in ("upper", "lower"):
        ...
        shape.exportStl(
            str(directories["print"] / f"captured-enclosure-{half}.stl"),
            tolerance=0.08,
    ...
    for variant in ("definitive", "prototype"):
        ...
        shape.exportStl(
            str(directories["print"] / f"captured-enclosure-pawl-{variant}.stl"),
            tolerance=0.05,
    ...
    manifest = {
        ...
        "parameters": {
            ...
            "shaft_head_recess_depth": (
                params.drum_enclosure.shaft_head_recess_depth
            ),
