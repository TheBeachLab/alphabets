...
| 64-position laser-cut drum | `../../Prototype/structure/spool.scad` | Current dimensional design |
| Lightweight printed spool | `../../Prototype/structure/spool-3dp.scad` | Legacy manufacturing alternative; validate physically |
| 28BYJ-48 motor | `../../Prototype/structure/28byj48.scad` | Clearance reference only |
| Holder side and inset | dormant `side()` and active `inset()` in `../../Prototype/structure/spool-holder.scad` | Legacy reference; validate physically |
| Enclosure sketch | `Sketch` in `../../Prototype/structure/side_motor.FCStd` | Exact non-solid reference geometry |
...
`../../Prototype/structure/connector.scad` is an empty placeholder, so it contains no
...
part. Its meshes remain in `../../Prototype/structure/` as historical reference. The
...
```sh
cd V2/Hardware/Final/mechanical
make setup
make check
```
